...
//			collector.add(LoseR1Constants.EPSILON);
		...
		for(RuleStateSet set : groups) {
			...
			for(RuleState state : set) {
				if(state.position >= state.rule.to.length()) {
					...
					if(state.rule.from.equals(LRAnalyzerConstants.UNIVERSE)) {
						action = ActionTable.ACCEPT;
					}

					// ???
					Set<String> following = LRAnalyzerUtil.generateFollowingLanguage(state, rules);
	...
	private static final Logger logger = LoggerFactory.createLogger(LRAnalyzerUtil.class);
}
